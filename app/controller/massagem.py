...
def adicionar_cliente(nome):
    ...
    conn.execute("INSERT INTO cliente_massagem (nome, status, checkins) VALUES (?, 0, 0)", (nome,))
    conn.commit()
    conn.close()

def excluir_cliente(cliente_id):
    conn = get_db_connection()
    conn.execute("DELETE FROM cliente_massagem WHERE id = ?", (cliente_id,))
...
def excluir_checkin(checkin_id):
    ...
    conn.execute("DELETE FROM checkin_massagem WHERE id = ?", (checkin_id,))
    status = False
